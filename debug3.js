...
console.log('Returned members length:', members.length);
if (members.length === 0) {
    console.log("Wait, why 0?");
    const mAll = db.prepare('SELECT * FROM members WHERE workspace_id = ?').all(workspaceId);
    console.log(mAll);
    const uAll = db.prepare('SELECT * FROM users').all();
    console.log(uAll);
}
