...
export async function POST(request, { params }) {
  try {
    ...
    await db.prepare('INSERT INTO comments (id, task_id, user_id, content, created_at) VALUES (?,?,?,?,?)').run(commentId, id, user.id, content, now);
    ...
    if (mentions.length > 0) {
      ...
      const foundUsers = await db.prepare(`SELECT id, name FROM users WHERE LOWER(REPLACE(name, ' ', '')) IN (${placeholders})`).all(...mentions);
    ...
    for (const uid of uniqueDefaultNotify) {
    ...
    const comment = await db.prepare('SELECT c.*, u.name as user_name, u.email as user_email FROM comments c JOIN users u ON c.user_id = u.id WHERE c.id = ?').get(commentId);
