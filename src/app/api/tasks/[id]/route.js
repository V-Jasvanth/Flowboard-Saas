...
export async function GET(request, { params }) {
  try {
    ...
    const labels = db.prepare('SELECT l.* FROM task_labels tl JOIN labels l ON tl.label_id = l.id WHERE tl.task_id = ?').all(id);
...
export async function PATCH(request, { params }) {
  try {
    ...
    const updates = [];
    const values = [];
    const project = db.prepare('SELECT workspace_id FROM projects WHERE id = ?').get(existing.project_id);
    ...
    fields.forEach(field => {
      if (body[field] !== undefined) {
        ...
        if (field === 'column_id' && body[field] !== existing.column_id) {
          const oldCol = db.prepare('SELECT name FROM columns WHERE id = ?').get(existing.column_id);
          const newCol = db.prepare('SELECT name FROM columns WHERE id = ?').get(body[field]);
    ...
    if (body.labels !== undefined) {
      ...
      body.labels.forEach(lid => ins.run(id, lid));
    }
