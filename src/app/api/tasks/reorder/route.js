    ...
    const reorder = db.transaction(() => {
      if (oldColumnId === targetColumnId) {
        if (newPosition > task.position) {
          db.prepare('UPDATE tasks SET position = position - 1 WHERE column_id = ? AND position > ? AND position <= ?').run(targetColumnId, task.position, newPosition);
        } else {
          db.prepare('UPDATE tasks SET position = position + 1 WHERE column_id = ? AND position >= ? AND position < ?').run(targetColumnId, newPosition, task.position);
        }
      } else {
        db.prepare('UPDATE tasks SET position = position - 1 WHERE column_id = ? AND position > ?').run(oldColumnId, task.position);
        db.prepare('UPDATE tasks SET position = position + 1 WHERE column_id = ? AND position >= ?').run(targetColumnId, newPosition);
      }
      db.prepare('UPDATE tasks SET column_id = ?, position = ?, updated_at = ? WHERE id = ?').run(targetColumnId, newPosition, new Date().toISOString(), taskId);
    });
    reorder();

    if (oldColumnId !== targetColumnId) {
      const oldCol = db.prepare('SELECT name FROM columns WHERE id = ?').get(oldColumnId);
      const newCol = db.prepare('SELECT name FROM columns WHERE id = ?').get(targetColumnId);
